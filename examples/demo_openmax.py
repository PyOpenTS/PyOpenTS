...
x_train, y_train, x_test, y_test = TSDataset(dataset_name='CBF', dataset_root_path='UCR', datasets_name='ucr').load()
...
# construct fcn model 
model = FCN(in_channels=x_train.shape[-1], unit_list=[64, 128], out_channels=3, num_classes=3, num_cnns=3, stride=1, padding='same')
model.to(device)
...
for epoch in tqdm(range(num_epochs)):
    ...
    for batch_inputs, batch_labels in train_dataloader:
        ...
        ave_loss = 0.8 * ave_loss + 0.2 * loss.item()
    ...
    with torch.no_grad():
        for batch_inputs, batch_labels in test_dataloader:
            ...
            test_loss = test_loss * 0.8 + loss.item() * 0.2
