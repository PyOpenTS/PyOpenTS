...
from opents.nn.models import BatchNormFCN
...
x_train, y_train, x_test, y_test = TSDataset(dataset_name=dataset_name, dataset_root_path='UCR', datasets_name='ucr').load()

x_train, y_train, x_val, y_val, x_test, y_test = RandomSplitOpenDataset(x_train=x_train, y_train=y_train, x_test=x_test, y_test=y_test, train_size_rate=0.6, test_size_rate=0.2, open_label_rate=0.2, train_random_state=42, open_random_state=42).load()
# [1, 3, 5]
...
for temp in np.arange(0.00001, 0.0005, 0.00001):
    ...
    # construct fcn model
    # 没有batchnorm 
    model = BatchNormFCN(in_channels=x_train.shape[-1], unit_list=[64, 128, 256], out_channels=num_classes, num_classes=num_classes, num_cnns=0, stride=1, padding="same")
    model.to(device)

    # loss funciton
    criterion = LogitNorm(device, temp) 
        ...
        if epoch % 100 == 0:
            ...
            if accuracy >= best_accuracy:
                if accuracy > best_accuracy or test_cls_loss < best_cls_loss:
                    ...
                    torch.save(model.state_dict(), f'{dataset_name}_LogitNorm_temp_simplemodel.pt')
